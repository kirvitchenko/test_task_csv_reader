...
def get_report(report_type, values_dict):
    """
    Получаем отсортированный список кортежей c данными по необходимому типу отчета
...
def main():
    ...
    table_data = [(i + 1, brand, rating) for i, (brand, rating) in enumerate(result)]

    print(
        tabulate(
            table_data, headers=["№", "Бренд", "Средний рейтинг"], tablefmt="fancy_grid"
        )
    )
